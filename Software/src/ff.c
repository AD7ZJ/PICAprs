...
#endif
#if _MAX_SS != 512
#define SS(fileSystem)  ((fileSystem)->ssize)   /* Variable sector size */
#else
#define SS(fileSystem)  512U            /* Fixed sector size */
#endif
...
#endif
#define ENTER_FF(fileSystem)        { if (!lock_fs(fileSystem)) return FR_TIMEOUT; }
#define LEAVE_FF(fileSystem, res)   { unlock_fs(fileSystem, res); return res; }
#else
#define ENTER_FF(fileSystem)
#define LEAVE_FF(fileSystem, res)   return res
#endif

#define ABORT(fileSystem, res)      { fp->err = (BYTE)(res); LEAVE_FF(fileSystem, res); }
...
#endif
typedef struct {
    FATFS *fileSystem;              /* Object ID 1, volume (NULL:blank entry) */
...
{
    return ff_req_grant(fileSystem->sobj);
}
...
)
{
    ...
    for (i = 0; i < _FS_LOCK; i++) {
        if (Files[i].fs == fileSystem) Files[i].fs = 0;
    }
}
#endif
...
#if _FS_REENTRANT
        FATFS *fileSystem = 0;
        if (res == FR_OK) fileSystem = fp->fs;  /* Get corresponding file system object */
#endif
        if (res == FR_OK) fp->fs = 0;   /* Invalidate file object */
        LEAVE_FF(fileSystem, res);
    }
#else
...
)
{
    ...
    /* Get logical drive number */
    res = find_volume(&fileSystem, &path, 0);
    if (res == FR_OK) {
        dp->fs = fileSystem;
        INIT_BUF(*dp);
        ...
        if (res == FR_OK) {                     /* Follow completed */
            if (dp->dir) {                      /* It is not the origin directory itself */
                if (dp->dir[DIR_Attr] & AM_DIR) /* The object is a sub directory */
                    dp->sclust = ld_clust(fileSystem, dp->dir);
...
)
{
    ...
    if (res == FR_OK) {
        ...
        if (fileSystem->free_clust <= fileSystem->n_fatent - 2) {
            *nclst = fileSystem->free_clust;
        } else {
            /* Get number of free clusters */
            fat = fileSystem->fs_type;
            n = 0;
            if (fat == FS_FAT12) {
                clst = 2;
                do {
                    stat = get_fat(fileSystem, clst);
                    if (stat == 0xFFFFFFFF) { res = FR_DISK_ERR; break; }
                    if (stat == 1) { res = FR_INT_ERR; break; }
                    if (stat == 0) n++;
                } while (++clst < fileSystem->n_fatent);
            } else {
                clst = fileSystem->n_fatent;
                sect = fileSystem->fatbase;
                i = 0; p = 0;
                do {
                    if (!i) {
                        res = move_window(fileSystem, sect++);
                        if (res != FR_OK) break;
                        p = fileSystem->win;
                        i = SS(fileSystem);
                    }
            ...
            fileSystem->free_clust = n;
            fileSystem->fsi_flag |= 1;
            *nclst = n;
        }
    }
    LEAVE_FF(fileSystem, res);
...
)
{
    ...
    fileSystem = FatFs[vol];
    if (!fileSystem) return FR_NOT_ENABLED;
    fileSystem->fs_type = 0;
    ...
    if (disk_ioctl(pdrv, GET_SECTOR_SIZE, &SS(fileSystem)) != RES_OK || SS(fileSystem) > _MAX_SS)
        return FR_DISK_ERR;
#endif
    if (_MULTI_PARTITION && part) {
        /* Get partition information from partition table in the MBR */
        if (disk_read(pdrv, fileSystem->win, 0, 1)) return FR_DISK_ERR;
        if (LD_WORD(fileSystem->win+BS_55AA) != 0xAA55) return FR_MKFS_ABORTED;
        tbl = &fileSystem->win[MBR_Table + (part - 1) * SZ_PTE];
    ...
    if (!au) {              /* AU auto selection */
        vs = n_vol / (2000 / (SS(fileSystem) / 512));
        for (i = 0; vs < vst[i]; i++) ;
        au = cst[i];
    }
    au /= SS(fileSystem);       /* Number of sectors per cluster */
    if (au == 0) au = 1;
    if (au > 128) au = 128;
    ...
    if (fmt == FS_FAT32) {
        n_fat = ((n_clst * 4) + 8 + SS(fileSystem) - 1) / SS(fileSystem);
        n_rsv = 32;
        n_dir = 0;
    } else {
        n_fat = (fmt == FS_FAT12) ? (n_clst * 3 + 1) / 2 + 3 : (n_clst * 2) + 4;
        n_fat = (n_fat + SS(fileSystem) - 1) / SS(fileSystem);
        n_rsv = 1;
        n_dir = (DWORD)N_ROOTDIR * SZ_DIR / SS(fileSystem);
    }
    ...
    if (_MULTI_PARTITION && part) {
        ...
        tbl = &fileSystem->win[MBR_Table + (part - 1) * SZ_PTE];
        tbl[4] = sys;
        if (disk_write(pdrv, fileSystem->win, 0, 1))    /* Write it to teh MBR */
            return FR_DISK_ERR;
        md = 0xF8;
    } else {
        if (sfd) {  /* No partition table (SFD) */
            ...
            mem_set(fileSystem->win, 0, SS(fileSystem));
            tbl = fileSystem->win+MBR_Table;    /* Create partition table for single partition in the drive */
            ...
            ST_WORD(fileSystem->win+BS_55AA, 0xAA55);   /* MBR signature */
            if (disk_write(pdrv, fileSystem->win, 0, 1))    /* Write it to the MBR */
                return FR_DISK_ERR;
            md = 0xF8;
        }
    }
    ...
    tbl = fileSystem->win;                          /* Clear sector */
    mem_set(tbl, 0, SS(fileSystem));
    mem_cpy(tbl, "\xEB\xFE\x90" "MSDOS5.0", 11);/* Boot jump code, OEM name */
    i = SS(fileSystem);                             /* Sector size */
    ...
    for (i = 0; i < N_FATS; i++) {      /* Initialize each FAT copy */
        mem_set(tbl, 0, SS(fileSystem));            /* 1st sector of the FAT  */
        ...
        if (disk_write(pdrv, tbl, wsect++, 1))
            return FR_DISK_ERR;
        mem_set(tbl, 0, SS(fileSystem));            /* Fill following FAT entries with zero */
